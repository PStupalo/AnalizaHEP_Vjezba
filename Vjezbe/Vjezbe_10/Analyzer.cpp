...
Analyzer::Analyzer() //konstruktor
{
}
Analyzer::~Analyzer() //destruktor
{
}

// f-ja za ChiSquareFit
void Analyzer::ChiSquareFit()
{
  //def novo platno i dijelimo ga na 2 dijela (opcionalno)
  c = new TCanvas("c","c",1200,800);
  c->Divide(2);
  gStyle->SetOptFit();

  c->cd(1); //prvi dio platna

//prvi zad - koristenje TGraphErrors za crtanje grafa
  double F[5]  = {1,2,3,4,5}; //podaci sila koje koristimo za mjerenje
  double a[5]  = {9.8,21.2,34.5,39.9,48.5}; //izmjerene akceleracije
  double sigma_F[5] = {0.,0.,0.,0.,0.}; //greske sila
  double sigma_a[5] = {1.0,1.9,3.1,3.9,5.1}; //greske akceleracija
  double sum1=0, sum2=0;
  int i;

//TGraphErrors koristi se za stvaranje grafa s pogreškama u mjerenjima.
  gr = new TGraphErrors(5,F,a,sigma_F,sigma_a); //5-br tocaka u grafu, F-na x, a- na y, odgovarajuca greska za x, greska za y

  gr->SetTitle("Masa kugle;F [N];a [m/s^{2}]");
  gr->SetMarkerColor(kBlack);
  gr->SetMarkerStyle(21);
  gr->Draw("AP"); // crta graf sa stilom "A" (Draw Axis) i "P" (Draw Points).
  ...
  c->cd(2); //drugi, desni, dio platna

  //graficko prikazivanje Chi-Square (zd4)
  ...
  for(i=0;i<5;i++) //metoda najmanjih kvadrata za dobivanje mase kuglice (predavanja)
   {
